...
export default function Favourites() {
  ...
  useEffect(() => {
      ...
      userFavourites = userFavourites.filter(fav => fav.id !== deletedFavBlogId);
    ...
    <div className="w-2/3 mx-auto bg-white p-6 rounded-xl shadow-lg">
      ...
      {allUsersFavouriteBlogs[currentSessionUser]?.length > 0 ? (
        allUsersFavouriteBlogs[currentSessionUser].map((blog, index) => (
          <RanderFavourites blog={blog} key={index} setRemoveFavourite={setRemoveFavourite} setDeletedFavBlogId={setDeletedFavBlogId}/>
        ))
      ) : (
