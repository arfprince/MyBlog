...
import RanderLiked from "./randerLiked";
...
export default function LastTenLikes() {
  ...
  if(allUserslikedBlogs[currentSessionUser] && allUserslikedBlogs[currentSessionUser].length <= 10){
  ...
  return (
    <div className="w-2/3 mx-auto bg-white p-6 rounded-xl shadow-lg">
      ...
      {userLastTenLikedBlogs?.length > 0 ? (
        ...
        <p className="text-gray-500 text-center mt-4">No liked blogs added yet.</p>
      )}
    </div>
  );
}
